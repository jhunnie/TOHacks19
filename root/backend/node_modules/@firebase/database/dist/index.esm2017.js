...
import { stringify, jsonEval, contains, assert, base64, stringToByteArray, Sha1, forEach, isNodeSdk, stringLength, errorPrefix, validateArgCount, validateCallback, Deferred, assertionError, safeGet, clone, map, getCount, getAnyKey, every, validateContextObject, isEmpty, getValues, findValue, findKey, deepCopy, isMobileCordova, base64Encode, CONSTANTS, isAdmin, isValidFormat, isReactNative, querystring } from '@firebase/util';
...
const parseRepoInfo = function (dataURL) {
    const parsedUrl = parseURL(dataURL), namespace = parsedUrl.subdomain;
 ...
 */
const parseURL = function (dataURL) {
...
function nodeFromJSON$1(json, priority = null) {
    ...
    if (!(json instanceof Array) && USE_HINZE) {
        ...
        forEach(hinzeJsonObj, (key, child) => {
            if (typeof key !== 'string' || key.substring(0, 1) !== '.') {
                // Ignore metadata nodes
                const childNode = nodeFromJSON$1(hinzeJsonObj[key]);
...
class SparseSnapshotTree {
    ...
    remember(path, data) {
        if (path.isEmpty()) {
            ...
            const child = this.children_.get(childKey);
            path = path.popFront();
    ...
    forget(path) {
        if (path.isEmpty()) {
            ...
            if (this.value_ !== null) {
                ...
                path = path.popFront();
                if (this.children_.contains(childKey)) {
                    const safeToRemove = this.children_.get(childKey).forget(path);
                    if (safeToRemove) {
...
class ImmutableTree {
    ...
    static fromObject(obj) {
        ...
        forEach(obj, (childPath, childSnap) => {
...
class TransportManager {
    ...
    initTransports_(repoInfo) {
        ...
        if (isSkipPollConnection) {
            ...
            each(TransportManager.ALL_TRANSPORTS, (i, transport) => {
...
class PersistentConnection extends ServerActions {
    ...
    restoreState_() {
        ...
        forEach(this.listens_, (pathString, queries) => {
            forEach(queries, (key, listenSpec) => {
                this.sendListen_(listenSpec);
            });
        });
        for (let i = 0; i < this.outstandingPuts_.length; i++) {
...
class Repo {
    ...
    update(path, childrenToMerge, onComplete) {
        ...
        forEach(childrenToMerge, (changedKey, changedValue) => {
            empty = false;
        ...
        if (!empty) {
            ...
            forEach(childrenToMerge, (changedPath) => {
    ...
    onDisconnectUpdate(path, childrenToMerge, onComplete) {
        ...
        this.server_.onDisconnectMerge(path.toString(), childrenToMerge, (status, errorReason) => {
            if (status === 'ok') {
                forEach(childrenToMerge, (childName, childNode) => {
...
class Tree {
     ...
     */
    forEachChild(action) {
        forEach(this.node_.children, (child, childTree) => {
...
class RepoManager {
    ...
    databaseFromApp(app, url) {
        const dbUrl = url || app.options[DATABASE_URL_OPTION];
        if (dbUrl === undefined) {
 ...
 * @return {!Object}
 */
const listens = function (firebaseRef) {
